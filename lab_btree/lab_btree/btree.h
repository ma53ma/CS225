...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
  ...
  if (elements[mid] == val) {
    ...
    std::vector<T> halfOne = std::vector<T>(elements.begin(), elements.begin() + mid);
    return insertion_idx(halfOne, val);
  } else {
    std::vector<T> halfTwo = std::vector<T>(elements.begin() + mid, elements.end());
    std::cout << halfTwo.size() << std::endl;
    return mid + insertion_idx(halfTwo, val);
  }
}
