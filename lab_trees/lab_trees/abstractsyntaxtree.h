...
: public BinaryTree<std::string>
{
    ...
    private:
        ...
        double mathStep(Node * node) const;

        double recursiveEval(Node * node) const;

        double twoUnknownMath(Node * node, Node * node1, Node * node2) const;

        double oneUnknownMath(Node * node, Node * node2, double known) const;
