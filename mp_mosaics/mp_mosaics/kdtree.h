...
class KDTree
{
  private:
    ...
    vector<Point<Dim>> realPoints;

    KDTreeNode * KDTreeHelper(vector<Point<Dim>>& realPoints, int left, int right, int dim);
    double eucDist(Point<Dim> first, Point<Dim> second) const;
    ...
    KDTreeNode * fNNHelper(typename KDTree<Dim>::KDTreeNode * root, const Point<Dim>& query, int dim) const;
    void clear(KDTreeNode * root);
    KDTreeNode * copy(KDTreeNode * subroot);
    KDTreeNode * copy(KDTree<Dim>& subtree);
    void clear();
