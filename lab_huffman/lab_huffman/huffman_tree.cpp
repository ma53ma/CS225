                            ...
                            queue<TreeNode*>& mergeQueue)
{
  if (mergeQueue.size() == 0) {
    TreeNode * returnptr = singleQueue.front();
    singleQueue.pop();
    return returnptr;
  } else if (singleQueue.size() == 0) {
    TreeNode * returnptr = mergeQueue.front();
    mergeQueue.pop();
    return returnptr;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     * for you.
     */
     for(vector<const Frequency>::iterator it = frequencies.begin(); it != frequencies.end(); ++it) {
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     if (!bfile.hasBits()) {
       ...
       Frequency newFreq = Frequency(bfile.getNextByte(), 0);
       TreeNode * newNode = new TreeNode (newFreq);
       return newNode;
     } else {
       Frequency newFreq = Frequency(0);
       TreeNode * newNode = new TreeNode (newFreq);
